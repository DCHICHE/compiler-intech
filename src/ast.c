...
ast_binary_e *ast_string_to_binary(char op)
{
  switch (op)
  {
  case '+': return AST_BIN_PLUS;
  case '-': return AST_BIN_MINUS;
  case '*': return AST_BIN_MULT;
  case '/': return AST_BIN_DIV;
  case 'ET': return AST_BIN_AND;
  case 'OU':return AST_BIN_OR;
  case '<':return AST_BIN_LT;
  case '<=':return AST_BIN_LTE;
  case '>':return AST_BIN_GT;
  case '>=':return AST_BIN_GTE;
  case '==':return AST_BIN_EQ;
  case '!=':return AST_BIN_DIFF;
  default:
    printf("unknown binary operator. exiting.\n");
    exit(1);
  }
